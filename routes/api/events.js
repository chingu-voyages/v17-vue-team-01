      ...
      { $push: {events: [event_id] }} ,function(err, user){
        ...
        if(doc){
          ...
          sgMail.send({
            to: user.email,
            from: 'chingutime@gmail.com',
            subject: 'You have been added to the event ' + event.title,
            // text: result.details,
            html: "<h3>You have been added to " + event.title + 
            " on <a href='https://chingutime.netlify.app'>Chingu Time App</a>! <br>Want to add your timeslots? Continue <a href='https://chingutime.netlify.app/#/event/" 
            + event_id + ">HERE</a></h3>"
          }, function(err, msg) {
            if(err) {
              return res.status(200).json({
