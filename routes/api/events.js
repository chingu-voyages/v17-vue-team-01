      ...
      { $push: {events: [event_id] }} ,function(err, user){
        ...
        if(doc){
          ...
          sgMail.send({
            ...
            // text: result.details,
            html: `'<h3>You have been added to '+ event.title + 'on ChinguTime! <br>Want to add your timeslots? Continue <a href="https://chingutime.netlify.com/#/event/`+ event_id+`">HERE</a></h3>'`
          }, function(err, msg) {
            if(err) {
              return res.status(200).json({
                success: false,
